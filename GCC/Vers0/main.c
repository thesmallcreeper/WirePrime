...
void nextgen(char* oldtable,char* newtable,int xsize,int ysize);
...
int main(int argc, char *argv[]) {
	FILE *inputfile= fopen(argv[1],"r");								//Reading file
	...
	char table[ysize+2][xsize+2];											//Creating and Filling table
	...
	SDL_Init(SDL_INIT_VIDEO);												//Create Window and show 1st generation
	...
	while(!done) {
		...
		int i;
		for(i=0;i<refreshevery;i++){									//Loop that create genetations
			nextgen(table,helptable,xsize,ysize);
...
void nextgen(char* oldtable,char* newtable,int xsize,int ysize){			//Create a new gen
	int i;
	for(i=1;i<ysize+1;i++){
		int j;
		for(j=1;j<xsize+1;j++){
			int counter=0;
			switch (*(oldtable+i*(xsize+2)+j)){
				case 1:
					*(newtable+i*(xsize+2)+j)=2;
					break;
				case 2:
					*(newtable+i*(xsize+2)+j)=3;
					break;
				case 3:
					if (*(oldtable+i*(xsize+2)+j-1)==1) counter++;
					if (*(oldtable+i*(xsize+2)+j+1)==1) counter++;
					if (*(oldtable+(i-1)*(xsize+2)+j)==1) counter++;
					if (*(oldtable+(i+1)*(xsize+2)+j)==1) counter++;
					if (*(oldtable+(i-1)*(xsize+2)+j-1)==1) counter++;
					if (*(oldtable+(i-1)*(xsize+2)+j+1)==1) counter++;
					if (*(oldtable+(i+1)*(xsize+2)+j-1)==1) counter++;
					if (*(oldtable+(i+1)*(xsize+2)+j+1)==1) counter++;
					
					if (counter==1 || counter==2) *(newtable+i*(xsize+2)+j)=1;
					else *(newtable+i*(xsize+2)+j)=3;
